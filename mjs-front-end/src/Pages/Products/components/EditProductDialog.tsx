...
const editProductFormSchema = z.object({
	code: z.coerce.number({invalid_type_error: "Apenas números."}).int("Apenas númeors inteiros."),
	...
	const {mutateAsync: editProductFn, isPending} = useMutation({
		...
		onMutate: (updatedRow) => {
			queryClient.setQueryData(["products"], (oldData: Product[]) =>
				oldData.map((row) => (row.code === updatedRow.code ? {...row, ...updatedRow} : row))
			);
		},
	});
			...
			<Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
				...
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>Edite o produto no estoque.</DialogDescription>
					</DialogHeader>
								...
								render={({field}) => (
									<FormControl>
										<FormItem>
											...
											<Input disabled type="number" className="remove-arrow" {...field} />
											<FormMessage />
										</FormItem>
									</FormControl>
								)}
